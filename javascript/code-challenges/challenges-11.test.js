...
const totalSum = (input) => {
  return input.reduce((arr, piece)=>arr=[...arr,...piece],[]).reduce((sum,num)=>sum=sum+num,0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(arr=>arr.filter(item=>typeof item === 'number'&&item % 5 === 0).map(num=>2**num));
};
