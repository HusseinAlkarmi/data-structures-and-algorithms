...
const screenForNames = (arr) => {
  // Solution code here...
  let r = /^(Mr||Mrs||Ms||Dr).\s\S[A-Za-z]*/gm;
  let array = [];
  arr.filter((item) => {
    if (item.match(r) !== null) {
      array.push(item.match(r)[0]);
    }
  });
  return array;
}
...
const toTitleCase = (arr) => {
  ...
  let Ar = [];
  arr.filter((value) => {
    let string = value.charAt(0).toUpperCase() + value.substr(1);
    Ar.push(string);
  });
  return Ar;
};
...
const sortBy = (property, arr) => {
  // Solution code here...
  let Ar = arr.sort((fEl, secEl) => {
    return fEl[property] > secEl[property] ? 1 : -1;
  });
  return Ar;
};
